...
of the maximum vacation hours for any employee. For example, 
...
SELECT
	...
	VacationHours,
	(SELECT MAX(VacationHours) FROM HumanResources.Employee) AS 'MaxVacationHours',
	VacationHours * 1.0 / (SELECT MAX(VacationHours) FROM HumanResources.Employee) AS 'PercentOfMaxHours'
FROM AdventureWorks2019.HumanResources.Employee
...
In other words, return only employees who have at least 80% as much 
...
SELECT
	...
	VacationHours * 1.0 / (SELECT MAX(VacationHours) FROM HumanResources.Employee) AS 'PercentOfMaxHours'
FROM AdventureWorks2019.HumanResources.Employee
WHERE VacationHours * 1.0 / (SELECT MAX(VacationHours) FROM HumanResources.Employee) >= .8
