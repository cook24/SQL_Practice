...
There should ONLY be three records per Vendor ID, 
even if some of the total amounts due are identical. 
...
SELECT
	...
	poh.Freight,
	poh.TotalDue,
	ROW_NUMBER() OVER(PARTITION BY poh.VendorID ORDER BY poh.TotalDue DESC) AS 'OrderRank'
...
Modify your query from the first problem, 
such that the top three purchase order amounts are returned, 
regardless of how many records are returned per Vendor Id.

In other words, 
if there are multiple orders with the same total due amount, 
all should be returned as long as the total due amount for these orders is one of the top three.

Ultimately, 
you should see three distinct total due amounts (i.e., the top three) 
for each group of like Vendor Ids. However, 
there could be multiple records for each of these amounts.


Hint: Think carefully about how the different ranking functions 
(ROW_NUMBER, RANK, and DENSE_RANK) work, 
...
SELECT
	...
	poh.Freight,
	poh.TotalDue,
	ROW_NUMBER() OVER(PARTITION BY poh.VendorID ORDER BY poh.TotalDue DESC) AS 'OrderTotalRank',
	DENSE_RANK() OVER(PARTITION BY poh.VendorID ORDER BY poh.TotalDue DESC) as 'Top3PriceTotals'
FROM Purchasing.PurchaseOrderHeader poh
) subq
WHERE Top3PriceTotals <= 3
